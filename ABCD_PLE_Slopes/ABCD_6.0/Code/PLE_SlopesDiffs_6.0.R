...
subject_counter <- PLEs %>% distinct(participant_id) %>% pull(participant_id) 
# 11867, missing one. which one?
all_subs <- sex %>% distinct(participant_id) %>% pull(participant_id)
missing <- setdiff(all_subs, subject_counter) # sub-LYKAB1UX (Male)
# redownloaded data to check, not a download error. This subject is not in the PLE spreadsheet.
...
one_tp <- Total_PLEs %>% filter(Timepoint_Count == 1) %>% left_join(sex, by = 'participant_id')
table(one_tp$sex) # 151 M, 175 F, 326 total
two_tp <- Total_PLEs %>% filter(Timepoint_Count == 2) %>% left_join(sex, by = 'participant_id')
table(two_tp$sex) # 168 M, 183 F, 351 total
threeplus_tp <- Total_PLEs %>% filter(Timepoint_Count > 2) %>% left_join(sex, by = 'participant_id')
table(threeplus_tp$sex) # 5870 M, 5320 F, 11190 total
# 326-1, 351-2, 11190-3+
...
slopes_count <- Total_PLEs %>% filter(!is.na(PLE_Slope)) %>% distinct(participant_id) %>% pull(participant_id)
diff_count <- Total_PLEs %>% filter(!is.na(PLE_Raw_Difference)) %>% distinct(participant_id) %>% pull(participant_id)
# 11190 slopes, 351 differences
total_count <- Total_PLEs %>% filter(!is.na(PLE_SlopeOrDiff)) %>% left_join(sex, by = 'participant_id')
table(total_count$sex) # 6038 M, 5503 F, 11541 total
