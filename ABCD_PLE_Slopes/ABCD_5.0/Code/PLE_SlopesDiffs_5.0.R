...
PLEs <- read_csv(paste0(datapath, '/mental-health/mh_y_pps.csv'))
Ages <- read_csv(paste0(datapath, '/abcd-general/abcd_y_lt.csv'))
...
sex <- sex %>% select(src_subject_id, demo_sex_v2) %>% rename(sex = demo_sex_v2)
...
# Count number of subjects in PLEs
subject_counter <- PLEs %>% distinct(src_subject_id) %>% pull(src_subject_id) 
...
# Count subjects with 1 timepoint (not enough data), 2 timepoints (will get differences), and 3+ timepoints (will get slopes)
table(Total_PLEs$Timepoint_Count)
no_tp <- Total_PLEs %>% filter(Timepoint_Count == 0) %>% left_join(sex, by = 'src_subject_id') # 1 Male
one_tp <- Total_PLEs %>% filter(Timepoint_Count == 1) %>% left_join(sex, by = 'src_subject_id')
table(one_tp$sex) # 158 M, 187 F, 345 total
two_tp <- Total_PLEs %>% filter(Timepoint_Count == 2) %>% left_join(sex, by = 'src_subject_id')
table(two_tp$sex) # 221 M, 251 F, 472 total
threeplus_tp <- Total_PLEs %>% filter(Timepoint_Count > 2) %>% left_join(sex, by = 'src_subject_id')
table(threeplus_tp$sex) # 5808 M, 5239 F, 3 IM, 11050 total
# 1-0tp 345-1tp, 472-2tp, 11050-3+tp, 11868 total
...
# Count Slopes and Differences
slopes_count <- Total_PLEs %>% filter(!is.na(PLE_Slope)) %>% distinct(src_subject_id) %>% pull(src_subject_id)
diff_count <- Total_PLEs %>% filter(!is.na(PLE_Raw_Difference)) %>% distinct(src_subject_id) %>% pull(src_subject_id)
# 11190 slopes, 351 differences
total_count <- Total_PLEs %>% filter(!is.na(PLE_SlopeOrDiff)) %>% left_join(sex, by = 'src_subject_id')
total_count %>% distinct(src_subject_id) %>% nrow()
table(total_count$sex) # 6029 M, 5490 F, 3 IM, 11522 total
...
write_csv(Total_PLEs, paste0(derivative_data, 'PLE_SlopesDiffs_12_9_25.csv'))
