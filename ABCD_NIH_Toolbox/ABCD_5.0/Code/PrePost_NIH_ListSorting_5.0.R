...
# remove events with no score data
nih <- nih %>%
...
# count subjects
parent_nih %>% filter(!is.na(last_pre_event) & !is.na(first_post_event)) %>% distinct(src_subject_id) %>% nrow() # 4011
youth_nih %>% filter(!is.na(last_pre_event) & !is.na(first_post_event)) %>% distinct(src_subject_id) %>% nrow() # 3833
...
# count subjects
parent_nih %>% filter(!is.na(ChangeScore)) %>% distinct(src_subject_id) %>% nrow() # 4011 -> 1873
youth_nih %>% filter(!is.na(ChangeScore)) %>% distinct(src_subject_id) %>% nrow() # 3833 -> 1823
...
write_csv(parent_nih, paste0(derivative_data, 'Parent_5.0_PrePost_NIH_LSWM_12_18_25.csv'))
write_csv(youth_nih, paste0(derivative_data, 'Youth_5.0_PrePost_NIH_LSWM_12_18_25.csv'))
...
parent_nih <- parent_nih %>% filter(!is.na(last_pre_event) & !is.na(first_post_event)) # 4011
youth_nih <- youth_nih %>% filter(!is.na(last_pre_event) & !is.na(first_post_event)) # 3833

# Parent - 4011 total
sum(!is.na(parent_nih$PreScore) & !is.na(parent_nih$PostScore)) # 1873 with both
sum(is.na(parent_nih$PreScore) | is.na(parent_nih$PostScore)) # 2138 missing at least one
sum(is.na(parent_nih$PreScore) & !is.na(parent_nih$PostScore)) # 28 missing just premenarche
sum(!is.na(parent_nih$PreScore) & is.na(parent_nih$PostScore)) # 2073 missing just postmenarche
sum(is.na(parent_nih$PreScore) & is.na(parent_nih$PostScore)) # 37 missing both premenarche and postmenarche

# Youth - 3833 total
sum(!is.na(youth_nih$PreScore) & !is.na(youth_nih$PostScore)) # 1823 with both
sum(is.na(youth_nih$PreScore) | is.na(youth_nih$PostScore)) # 2010 missing at least one
sum(is.na(youth_nih$PreScore) & !is.na(youth_nih$PostScore)) # 28 missing just premenarche
sum(!is.na(youth_nih$PreScore) & is.na(youth_nih$PostScore)) # 1950 missing just postmenarche
sum(is.na(youth_nih$PreScore) & is.na(youth_nih$PostScore)) # 32 missing both premenarche and postmenarche
