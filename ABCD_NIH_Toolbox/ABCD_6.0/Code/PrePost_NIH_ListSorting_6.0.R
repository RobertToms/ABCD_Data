...
# count sessions/subjects with no score data
no_nih <- nih %>%
  filter(is.na(nc_y_nihtb__lswmt__agecor_score))
num_subs_no_nih <- length(table(no_nih$participant_id)) # 5170 subjects affected

num_subs_nih <- length(table(nih$participant_id)) # 5675 subjects with NIH LSWM data

# remove sessions with no score data
nih <- nih %>%
  # Before: 17771 timepoints
  filter(!is.na(nc_y_nihtb__lswmt__agecor_score))
# After: 12311 timepoints
num_subs_has_nih <- length(table(nih$participant_id)) # 5658 female subjects with NIH LSWM data

# SANITY CHECK: 20 subjects missing, do they have NIH LSWM data?
f_sex <- sex %>% filter(ab_g_stc__cohort_sex == 2) # 5678 total females
missing_subs <- f_sex %>%
  left_join(nih, by = "participant_id") %>%
  filter(is.na(session_id) & is.na(nc_y_nihtb__lswmt_dtt) & is.na(nc_y_nihtb__lswmt__agecor_score)) # 20 subjects have no NIH LSWM data at all
...
write_csv(parent_nih, paste0(derivative_data, 'Parent_PrePost_NIHtbx_Menarche_12_1_25.csv'))
write_csv(youth_nih, paste0(derivative_data, 'Youth_PrePost_NIHtbx_Menarche_12_1_25.csv'))
...
# just those with valid pre/post menarche timepoints
parent_nih <- parent_nih %>% filter(!is.na(last_pre_session) & !is.na(first_post_session)) # 4912
youth_nih <- youth_nih %>% filter(!is.na(last_pre_session) & !is.na(first_post_session)) # 4812

# re-call nih data and menarche data without prior exclusions
nih <- read_tsv(paste0(sourcepath, 'Neurocognition/nc_y_nihtb.tsv')) %>%  select(participant_id, session_id, nc_y_nihtb__lswmt_dtt, nc_y_nihtb__lswmt__agecor_score)
parent <- read_csv(paste0(derivative_data, 'Parent_PrePost_Menarche_11_21_25.csv')) %>% select(participant_id, last_pre_session, first_post_session)
youth <- read_csv(paste0(derivative_data, 'Youth_PrePost_Menarche_11_21_25.csv')) %>% select(participant_id, last_pre_session, first_post_session)

## PARENT: 4923 -> 4912 = 11, why are they missing?
parent_has_nih <- parent %>% filter(!is.na(last_pre_session) & !is.na(first_post_session)) %>% distinct(participant_id) %>% pull(participant_id)
parent_has_prepost_nih <- parent_nih %>% distinct(participant_id) %>% pull(participant_id)
missing_p <- setdiff(parent_has_nih, parent_has_prepost_nih) # 11
missing_p_nih <- nih %>% filter(participant_id %in% missing_p)
missing_p_nih_subs <- missing_p_nih %>% distinct(participant_id) %>% pull(participant_id) # 11 subjects have no available data (all n/a's)

## YOUTH: 4822 -> 4812 = 10, why are they missing?
youth_has_nih <- youth %>% filter(!is.na(last_pre_session) & !is.na(first_post_session)) %>% distinct(participant_id) %>% pull(participant_id)
youth_has_prepost_nih <- youth_nih %>% distinct(participant_id) %>% pull(participant_id)
missing_y <- setdiff(youth_has_nih, youth_has_prepost_nih) # 10
missing_y_nih <- nih %>% filter(participant_id %in% missing_y)
missing_y_nih_subs <- missing_y_nih %>% distinct(participant_id) %>% pull(participant_id) # 10 subjects have no available data (all n/a's)


# Parent - 4912 total
sum(!is.na(parent_nih$PreScore) & !is.na(parent_nih$PostScore)) # 4161 with both
sum(is.na(parent_nih$PreScore) | is.na(parent_nih$PostScore)) # 751 missing at least one
sum(is.na(parent_nih$PreScore) & !is.na(parent_nih$PostScore)) # 64 missing just premenarche
sum(!is.na(parent_nih$PreScore) & is.na(parent_nih$PostScore)) # 687 missing just postmenarche
sum(is.na(parent_nih$PreScore) & is.na(parent_nih$PostScore)) # 0 missing both premenarche and postmenarche

# Youth - 4812 total
sum(!is.na(youth_nih$PreScore) & !is.na(youth_nih$PostScore)) # 4071 with both
sum(is.na(youth_nih$PreScore) | is.na(youth_nih$PostScore)) # 741 missing at least one
sum(is.na(youth_nih$PreScore) & !is.na(youth_nih$PostScore)) # 62 missing just premenarche
sum(!is.na(youth_nih$PreScore) & is.na(youth_nih$PostScore)) # 679 missing just postmenarche
sum(is.na(youth_nih$PreScore) & is.na(youth_nih$PostScore)) # 0 missing both premenarche and postmenarche
