...
subject_counter <- AllSalivaHormones %>% distinct(participant_id) %>% pull(participant_id) # 5676 subjects
# 2 females are gone, why?
females <- Demographics %>% filter(sex == 2) %>% distinct(participant_id) %>% pull(participant_id)
missing_two <- setdiff(females, subject_counter) # sub-0HB853U9, sub-U1T94DML 
# no salivary data for these two, redownloaded and checked data to confirm it's not a download error
...
sum(NumTimepointsPerSubject$TimepointsWithE2Data > 0) # 5647 subjects have at least 1 timepoint of E2 Data
sum(NumTimepointsPerSubject$TimepointsWithE2Data == 0) # 29 subjects have 0 timepoints with usable E2 Data
...
length(subject_counter$participant_id) # 5647 female subjects remain with valid e2 data 
...
# count subjects
subject_counter <- AllSalivaHormones %>% group_by(participant_id) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$participant_id) # 5642 female subjects remain with valid e2 data 
...
table(AllSalivaHormones$`Excluded?`) # n = 776 timepoints
...
length(subject_counter$participant_id) # 5641 female subjects remain with valid e2 data, 1 removed
...
table(AllSalivaHormones$`Excluded?`) # n = 3195, 2419 timepoints excluded

# count remaining subjects
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(participant_id) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$participant_id) # 5641 female subjects remain with valid e2 data, 0 removed
...
table(AllSalivaHormones$`Excluded?`) # n = 3851, 656 timepoints excluded
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(participant_id) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$participant_id) # 5639 female subjects remain with valid e2 data, 2 removed
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(participant_id) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$participant_id) # 5639 female subjects remain with valid e2 data, 0 removed
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(participant_id) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$participant_id) # 5637 female subjects remain with valid e2 data, 2 removed
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(participant_id) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$participant_id) # 5637 female subjects remain with valid e2 data, 0 removed
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(participant_id) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$participant_id) # 5636 female subjects remain with valid e2 data, 1 removed
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(participant_id) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$participant_id) # 5635 female subjects remain with valid e2 data, 1 removed
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(participant_id) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$participant_id) # 5634 female subjects remain with valid e2 data, 1 removed
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(participant_id) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$participant_id) # 5633 female subjects remain with valid e2 data, 1 removed
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(participant_id) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$participant_id) # 5626 female subjects remain with valid e2 data, 7 removed
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(participant_id) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$participant_id) # 5623 female subjects remain with valid e2 data, 3 removed
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(participant_id) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$participant_id) # 5621 female subjects remain with valid e2 data, 2 removed
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(participant_id) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$participant_id) # 5600 female subjects remain with valid e2 data, 21 removed
...
# count subjects - Aim:  5600
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(participant_id) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$participant_id) # 5600 female subjects with valid e2 data

# Aim: 5357 timepoints excluded
...
# Write to .csv
write_csv(AllSalivaHormones, paste0(derivative_data, "E2_Exclusions_11_20_25.csv"))
