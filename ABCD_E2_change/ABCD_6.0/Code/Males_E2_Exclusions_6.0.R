...
# read in csv files
saliva <- read_tsv(paste0(sourcepath, "Physical_Health/ph_y_phs.tsv"))
...
# count sex
table(Demographics$sex) # 6190 M, 5678 F , 11868 total

# Remove Males
AllSalivaHormones <- AllSalivaHormones %>%
  # before: 50155 
  filter(sex == 1)
# after: 26194, 23961 removed

# count subjects again
subject_counter <- AllSalivaHormones %>% distinct(participant_id) %>% pull(participant_id) # 6187 subjects
# 3 males are gone, why?
males <- Demographics %>% filter(sex == 1) %>% distinct(participant_id) %>% pull(participant_id)
missing_three <- setdiff(males, subject_counter) # sub-47N6G712, sub-VCAHYYKD, sub-ZH1A3UUP
# no salivary data for these three, redownloaded and checked data to confirm it's not a download error
...
bc_999 <- AllSalivaHormones %>%            
  filter(BirthControlStatus == 999) # 0 tps to be set to NA
bc_777 <- AllSalivaHormones %>%            
  filter(BirthControlStatus == 777) # 0 tps to be set to NA
bc_999_subjects <- length(table(bc_999$participant_id)) # 0 subjects affected
bc_777_subjects <- length(table(bc_777$participant_id)) # 0 subjects affected
...
# Sanity check -- compute E2 means by averaging the two samples, or use 1 if only 1 is available.
AllSalivaHormones <- AllSalivaHormones %>%
  # convert to numeric
...
NumTimepointsPerSubject <- AllSalivaHormones %>%
...
# Count number of subjects with at least 1 E2 Sample
sum(NumTimepointsPerSubject$TimepointsWithE2Data > 0) # 129 subjects have at least 1 timepoint of E2 Data
sum(NumTimepointsPerSubject$TimepointsWithE2Data == 0) # 6058 subjects have 0 timepoints with usable E2 Data

# remove subjects with no data
AllSalivaHormones <- AllSalivaHormones %>%
  filter(!is.na(E2mean))

# recount remaining subjects
AllSalivaHormones %>% distinct(participant_id) %>% nrow() # 129 subjects
...
# count subjects
subject_counter <- AllSalivaHormones %>% group_by(participant_id) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$participant_id) # 129 male subjects remain with valid e2 data 
...
table(AllSalivaHormones$E1) # 0
table(AllSalivaHormones$E2) # 5
AllSalivaHormones <- AllSalivaHormones %>%
  mutate(step_2_3 = if_else(E1 == 1 | E2 == 1, 1, 0))
table(AllSalivaHormones$step_2_3) # 5
...
AllSalivaHormones <- AllSalivaHormones %>%
  # make flags for if it is below or above the limits, so these can be counted
  mutate(below_detect = if_else(E2_Rep1 < 0.1 | E2_Rep2 < 0.1, 1, 0)) %>%
  mutate(above_detect = if_else(E2_Rep1 > 32 | E2_Rep2 > 32, 1, 0))
table(AllSalivaHormones$below_detect) # 0
table(AllSalivaHormones$above_detect) # 0
# make flag for if a sample is below or above limits
...
# Flag Sample flag (E1 or E2) if below or above limits
...
table(AllSalivaHormones$`Excluded?`) # n = 3 timepoints

# count remaining subjects
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(participant_id) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$participant_id) # 126 male subjects remain with valid e2 data, 3 removed
...
table(AllSalivaHormones$`Excluded?`) # n = 3, 0 timepoints excluded

# count remaining subjects
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(participant_id) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$participant_id) # 126 male subjects remain with valid e2 data, 0 removed
...
# MALES LIKELY NOT ON BIRTH CONTROL
...
table(AllSalivaHormones$`Excluded?`) # n = 3, 0 excluded
...
table(AllSalivaHormones$no_wake) # 6
table(AllSalivaHormones$no_start) # 0
table(AllSalivaHormones$no_end) # 0
table(AllSalivaHormones$no_freeze) # 0

#### Flag for exclusion and count
## No Wake Time
AllSalivaHormones$`Excluded?` <- if_else(AllSalivaHormones$no_wake == 1, 1, AllSalivaHormones$`Excluded?`)
table(AllSalivaHormones$`Excluded?`) # n = 9, 6 excluded

# count remaining subjects
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(participant_id) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$participant_id) # 122 male subjects remain with valid e2 data, 4 removed

## No Start Time
AllSalivaHormones$`Excluded?` <- if_else(AllSalivaHormones$no_start == 1, 1, AllSalivaHormones$`Excluded?`)
table(AllSalivaHormones$`Excluded?`) # n = 9, 0 excluded

# count remaining subjects
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(participant_id) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$participant_id) # 122 male subjects remain with valid e2 data, 0 removed
...
table(AllSalivaHormones$`Excluded?`) # n = 11, 2 excluded

# count remaining subjects
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(participant_id) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$participant_id) # 120 male subjects remain with valid e2 data, 2 removed
...
table(AllSalivaHormones$`Excluded?`) # n = 12, 1 excluded

# count remaining subjects
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(participant_id) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$participant_id) # 119 male subjects remain with valid e2 data, 1 removed
...
table(AllSalivaHormones$`Excluded?`) # n = 12, 0 excluded

# count remaining subjects
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(participant_id) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$participant_id) # 119 male subjects remain with valid e2 data, 0 removed
...
AllSalivaHormones$`Excluded?` <- if_else(
  AllSalivaHormones$QualCon == 1 |
    ...
    AllSalivaHormones$BirthControlStatus == 1 | # Chaku & Barry used BirthConNA, not BirthControlStatus. See note in line 387.
...
# count subjects - Aim:  119
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(participant_id) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$participant_id) # 119 female subjects with valid e2 data

# Aim: 12 timepoints excluded
table(AllSalivaHormones$`Excluded?`)
# Total: 12 excluded, 131 usable, 143 total
...
# 12 of 12 timepoints excluded for a single reason
...
# Write to .csv
write_csv(AllSalivaHormones, paste0(derivative_data, "Males_E2_Exclusions_12_9_25.csv"))


#######################
# IN THE BELOW CODE BLOCK
# bind males to females, make combined spreadsheet
########################

males <- AllSalivaHormones

# Rerun E2_Exclusions_6.0.R to set AllSalivaHormones to be all Female data

males$sex <- "M"
AllSalivaHormones$sex <- "F"

both <- AllSalivaHormones %>%
  rbind(males)

both_counts <- both %>% select(participant_id, sex, `Excluded?`) %>% filter(`Excluded?` == 0) %>% unique()
table(both_counts$sex) # 5600 F / 119 M, sanity checked

write_csv(both, paste0(derivative_data, 'MalesFemales_E2_Exclusions_12_9_25.csv'))