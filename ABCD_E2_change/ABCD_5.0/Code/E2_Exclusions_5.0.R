...
AllSalivaHormones <- AllSalivaHormones %>%
  select(src_subject_id, 
         eventname, 
         hormone_sal_sex, 		# Salivary Sex; Female = 1, Male = 2; Participant unable to complete = 3; Participant/Parent refused = 4; Institution not authorized to collect samples due to COVID = 6; Staff discomfort due to COVID = 7; Exceed 15min of < 6ft distance from youth (COVID) = 8; Not approved by ABCD for collection = 9; Not collected (other) = 5
...
# count MasterSex
table(Demographics$MasterSex) # 6188 M, 5677 F, 3 Intersex M, 11868 total
...
# sanity check -- count subjects again
subject_counter <- AllSalivaHormones %>% distinct(SubjectID) %>% pull(SubjectID) # 5677 subjects

# Merge BirthControl into AllSalivaHormones
AllSalivaHormones <- AllSalivaHormones %>%
  left_join(BirthControl, by = c("SubjectID", "EventName"))

## Count subjects with 3s or 2s in Birth Control Reports -- Are you currently using hormonal birth control? 1 = Yes; 0 = No; 2 = Don't know; 3 = Refuse to answer
bc_2 <- AllSalivaHormones %>%            
  filter(BirthControlStatus == 2) # 61 tps to be set to NA
bc_3 <- AllSalivaHormones %>%            
  filter(BirthControlStatus == 3) # 101 tps to be set to NA
bc_2_subjects <- length(table(bc_2$SubjectID)) # 59 subjects affected
bc_3_subjects <- length(table(bc_3$SubjectID)) # 95 subjects affected
...
sum(NumTimepointsWithData$TimepointsWithE2Data > 0) # 5638 subjects have at least 1 timepoint of E2 Data
sum(NumTimepointsWithData$TimepointsWithE2Data == 0) # 39 subjects have 0 timepoints with usable E2 Data
# 5677 - 5638 = 39 subjects with no usable e2 samples
...
# count subjects
subject_counter <- AllSalivaHormones %>% group_by(SubjectID) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$SubjectID) # 5638 female subjects remain with valid e2 data 
...
# count subjects excluded for this flag
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(SubjectID) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$SubjectID) # 5638 female subjects remain with valid e2 data, 0 removed
...
table(AllSalivaHormones$below_detect) # 200
...
table(AllSalivaHormones$step_4_5) # 200
...
AllSalivaHormones <- AllSalivaHormones %>%
  select(-settoNA) # 2628 samples set to NA
...
# count subjects
subject_counter <- AllSalivaHormones %>% group_by(SubjectID) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$SubjectID) # 5630 female subjects remain with valid e2 data, 8 removed
...
# sanity check -- Exclude Mismatched MasterSex and Salivary Sex
AllSalivaHormones$`Excluded?` <- if_else(AllSalivaHormones$`Mas!=Sal` == 1, 1, 0)
table(AllSalivaHormones$`Excluded?`) # n = 1090 timepoints excluded
# count subjects excluded for this flag
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(SubjectID) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$SubjectID) # 5630 female subjects remain with valid e2 data, 0 removed
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(SubjectID) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$SubjectID) # 5627 female subjects remain with valid e2 data, 3 removed
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(SubjectID) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$SubjectID) # 5627 female subjects remain with valid e2 data, 0 removed
...
AllSalivaHormones$BirthConNA <- if_else(AllSalivaHormones$MasterSex == 1 & is.na(AllSalivaHormones$BirthControlStatus), 1, 0)
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(SubjectID) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$SubjectID) # 5625 female subjects remain with valid e2 data, 2 removed
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(SubjectID) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$SubjectID) # 5625 female subjects remain with valid e2 data, 0 removed
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(SubjectID) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$SubjectID) # 5624 female subjects remain with valid e2 data, 1 removed
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(SubjectID) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$SubjectID) # 5623 female subjects remain with valid e2 data, 1 removed
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(SubjectID) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$SubjectID) # 5621 female subjects remain with valid e2 data, 2 removed
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(SubjectID) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$SubjectID) # 5618 female subjects remain with valid e2 data, 3 removed
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(SubjectID) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$SubjectID) # 5608 female subjects remain with valid e2 data, 10 removed
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(SubjectID) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$SubjectID) # 5604 female subjects remain with valid e2 data, 4 removed
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(SubjectID) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$SubjectID) # 5601 female subjects remain with valid e2 data, 3 removed
...
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(SubjectID) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$SubjectID) # 5574 female subjects remain with valid e2 data, 27 removed
...
# count subjects - Aim:  5574
subject_counter <- AllSalivaHormones %>% rowwise() %>% mutate(E2mean = if_else(`Excluded?` == 1, NA, E2mean)) %>%
  group_by(SubjectID) %>% summarise(TimepointsWithE2Data = sum(!is.na(E2mean))) %>% filter(TimepointsWithE2Data > 0)
length(subject_counter$SubjectID) # 5574 female subjects with valid e2 data
...
length(SingleExclusions$SubjectID)
# 1638 of 3117 timepoints excluded for a single reason
...
# Write to .csv
write_csv(AllSalivaHormones, paste0(derivative_data, "E2_Exclusions_5.0_12_5_25.csv"))
