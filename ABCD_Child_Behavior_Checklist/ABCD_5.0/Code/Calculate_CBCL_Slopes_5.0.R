...
sex <- sex %>% select(src_subject_id, demo_sex_v2) %>% filter(!is.na(demo_sex_v2)) %>% rename(sex = demo_sex_v2)


# Count number of subjects in CBCL
subject_counter <- CBCL %>% distinct(src_subject_id) %>% pull(src_subject_id) 
# 11866, missing two. which two?
all_subs <- sex %>% distinct(src_subject_id) %>% pull(src_subject_id)
missing <- setdiff(all_subs, subject_counter) # NDAR_INVC2ZW147D (Male), NDAR_INVFZWKRBW7 (Female)
...
int_one <- Internalizing_CBCL %>% filter(Timepoint_Count == 1) %>% left_join(sex, by = 'src_subject_id')
table(int_one$sex) # 204 M / 247 F, 451 total
ext_one <- Externalizing_CBCL %>% filter(Timepoint_Count == 1) %>% left_join(sex, by = 'src_subject_id')
table(ext_one$sex) # 204 M / 247 F, 451 total
int_two <- Internalizing_CBCL %>% filter(Timepoint_Count == 2) %>% left_join(sex, by = 'src_subject_id')
table(int_two$sex) # 1727 M / 1554 F / 2 IM, 3283 total
ext_two <- Externalizing_CBCL %>% filter(Timepoint_Count == 2) %>% left_join(sex, by = 'src_subject_id')
table(ext_two$sex) # 1727 M / 1554 F / 2 IM, 3283 total
int_threeplus <- Internalizing_CBCL %>% filter(Timepoint_Count > 2) %>% left_join(sex, by = 'src_subject_id')
table(int_threeplus$sex) # 4256 M / 3875 F / 1 IM, 8132 total
ext_threeplus <- Externalizing_CBCL %>% filter(Timepoint_Count > 2) %>% left_join(sex, by = 'src_subject_id')
table(ext_threeplus$sex) # 4256 M / 3875 F / 1 IM, 8132 total
...
int_final_counts <- Internalizing_CBCL %>% filter(!is.na(Int_Slope_or_Diff)) %>% left_join(sex, by = 'src_subject_id')
table(int_final_counts$sex) # 5983 M / 5429 F / 3 IM, 11415 total
ext_final_counts <- Externalizing_CBCL %>% filter(!is.na(Ext_Slope_or_Diff)) %>% left_join(sex, by = 'src_subject_id')
table(ext_final_counts$sex) # 5983 M / 5429 F / 3 IM, 11415 total
...
write_csv(Internalizing_CBCL, paste0(derivative_data, 'Int_CBCL_SlopeDiff_12_18_25.csv'))
write_csv(Externalizing_CBCL, paste0(derivative_data, 'Ext_CBCL_SlopeDiff_12_18_25.csv'))
