...
# The purpose of this code is to calculate changes in CBCL Scores from pre to post menarche.
...
sex <- sex %>%
  ...
  filter(sex == 2) # keep only females - 5678 total
...
# count subjects
int_subs <- cbcl_int %>% distinct(participant_id) %>% pull(participant_id) # 5677
ext_subs <- cbcl_ext %>% distinct(participant_id) %>% pull(participant_id) # 5677

# 1 subject missing, who and why?
all_females <- sex %>% pull(participant_id)
int_missing <- setdiff(all_females, int_subs)
ext_missing <- setdiff(all_females, ext_subs)
# sub-FZWKRBW7, no CBCL data, no parent menarche reports, no postmenarche youth report
...
write_csv(int_youth, paste0(derivative_data, 'Youth_PrePost_IntCBCL_12_1_25.csv'))
write_csv(ext_youth, paste0(derivative_data, 'Youth_PrePost_ExtCBCL_12_1_25.csv'))
write_csv(int_parent, paste0(derivative_data, 'Parent_PrePost_IntCBCL_12_1_25.csv'))
write_csv(ext_parent, paste0(derivative_data, 'Parent_PrePost_ExtCBCL_12_1_25.csv'))
...
int_y_counter <- int_youth %>%
  filter(!is.na(last_pre_session) & !is.na(first_post_session)) %>%
...
table(int_y_counter$has_changescore) # 4782 with, 40 without
table(ext_y_counter$has_changescore) # 4782 with, 40 without
table(int_p_counter$has_changescore) # 4921 with, 2 without
table(ext_p_counter$has_changescore) # 4921 with, 2 without

# Youth: 4822 -> 4782
# Parent: 4923 -> 4921

# determine reasons for no changescore
int_y_excluded <- int_y_counter %>% filter(has_changescore == FALSE) # 40 subjects, 39 no PostMenarche CBCL data, 1 no PreMenarche CBCL data
ext_y_excluded <- ext_y_counter %>% filter(has_changescore == FALSE) # 40 subjects, 39 no PostMenarche CBCL data, 1 no PreMenarche CBCL data
int_p_excluded <- int_p_counter %>% filter(has_changescore == FALSE) # 2 subjects, no PostMenarche CBCL Data
ext_p_excluded <- ext_p_counter %>% filter(has_changescore == FALSE) # 2 subjects, no PostMenarche CBCL Data
