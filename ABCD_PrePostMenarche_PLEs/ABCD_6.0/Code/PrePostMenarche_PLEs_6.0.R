...
# Parent: 4923 subjects with Pre/Post Menarche Timepoints
# Youth: 4822 subjects with Pre/Post Menarche Timepoints
...
# Parent: n = 0 premenarche flags, n = 3 postmenarche flags, all are postmenarche after last ple score report
# Youth: n = 1 premenarche flag, no premenarche ple data. n = 0 postmenarche flags
...
table(p_counter$has_changescore) # 4920

y_counter <- youth_ples %>%
  mutate(has_changescore = !is.na(ChangeScore))
table(y_counter$has_changescore) # 4821
...
write_csv(parent_ples_dates, paste0(derivative_data, 'Parent_PrePost_PLEs_12_1_25.csv'))
write_csv(youth_ples_dates, paste0(derivative_data, 'Youth_PrePost_PLEs_12_1_25.csv'))
