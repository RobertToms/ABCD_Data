...
table(sex$demo_sex_v2) # 6188 M, 5677 F, 3 IM;
...
# Count subjects
counter <- hormones %>% select(src_subject_id, MasterSex) %>% unique()
table(counter$MasterSex) # 6187 M, 5677 F, 3 IM; 1M removed, who?
hormone_subs <- hormones %>% distinct(src_subject_id) %>% pull(src_subject_id)
all_subs <- sex %>% distinct(src_subject_id) %>% pull(src_subject_id)
missing <- setdiff(all_subs, hormone_subs) # NDAR_INVLK7A64VC, no hormone data available
...
# Count subjects with usable DHEA Samples
counter <- hormones %>% group_by(src_subject_id) %>% mutate(total_samples = sum(SamplesPerVisit)) %>% select(src_subject_id, MasterSex, total_samples) %>% filter(total_samples > 0) %>% unique()
table(counter$MasterSex) # 6167 M, 5666 F, 3 IM; -20M, -11F
...
# Count subjects with valid samples remaining
counter <- hormones %>% group_by(src_subject_id) %>% mutate(total_samples = sum(SamplesPerVisit)) %>% select(src_subject_id, MasterSex, total_samples) %>% filter(total_samples > 0) %>% unique()
table(counter$MasterSex) # 6167 M, 5666 F, 3 IM;
                         # 6093 M, 5634 F, 2 IM; -74M, -32F, -1IM, -107 total
...
# count subjects
subject_counter <- hormones %>% rowwise() %>% mutate(DHEAmean = if_else(`Excluded?` == 1, NA, DHEAmean)) %>% group_by(src_subject_id) %>% summarise(TimepointsWithDHEAData = sum(!is.na(DHEAmean))) %>% filter(TimepointsWithDHEAData > 0) %>% left_join(sex, by = 'src_subject_id')
table(subject_counter$MasterSex) # 6089 Males, 5632 Females remain, 11721 total. -2F, -4M, -2 IM, -8 total
...
# count subjects
subject_counter <- hormones %>% rowwise() %>% mutate(DHEAmean = if_else(`Excluded?` == 1, NA, DHEAmean)) %>% group_by(src_subject_id) %>% summarise(TimepointsWithDHEAData = sum(!is.na(DHEAmean))) %>% filter(TimepointsWithDHEAData > 0) %>% left_join(sex, by = 'src_subject_id')
table(subject_counter$MasterSex) # 6085 Males, 5631 Females remain, 11716 total. -1F, -4M, -5 total
...
# count subjects
subject_counter <- hormones %>% rowwise() %>% mutate(DHEAmean = if_else(`Excluded?` == 1, NA, DHEAmean)) %>% group_by(src_subject_id) %>% summarise(TimepointsWithDHEAData = sum(!is.na(DHEAmean))) %>% filter(TimepointsWithDHEAData > 0) %>% left_join(sex, by = 'src_subject_id')
table(subject_counter$MasterSex) # 6085 Males, 5631 Females remain, 11716 total. -0 total
...
# count subjects
subject_counter <- hormones %>% rowwise() %>% mutate(DHEAmean = if_else(`Excluded?` == 1, NA, DHEAmean)) %>% group_by(src_subject_id) %>% summarise(TimepointsWithDHEAData = sum(!is.na(DHEAmean))) %>% filter(TimepointsWithDHEAData > 0) %>% left_join(sex, by = 'src_subject_id')
table(subject_counter$MasterSex) # 6084 Males, 5629 Females remain, 11713 total. -2F, -1M, -3 total
...
# count subjects
subject_counter <- hormones %>% rowwise() %>% mutate(DHEAmean = if_else(`Excluded?` == 1, NA, DHEAmean)) %>% group_by(src_subject_id) %>% summarise(TimepointsWithDHEAData = sum(!is.na(DHEAmean))) %>% filter(TimepointsWithDHEAData > 0) %>% left_join(sex, by = 'src_subject_id')
table(subject_counter$MasterSex) # 6084 Males, 5629 Females remain, 11713 total. -0 total
...
# count subjects
subject_counter <- hormones %>% rowwise() %>% mutate(DHEAmean = if_else(`Excluded?` == 1, NA, DHEAmean)) %>% group_by(src_subject_id) %>% summarise(TimepointsWithDHEAData = sum(!is.na(DHEAmean))) %>% filter(TimepointsWithDHEAData > 0) %>% left_join(sex, by = 'src_subject_id')
table(subject_counter$MasterSex) # 6083 Males, 5629 Females remain, 11712 total. -1M total

hormones$`Excluded?` <- if_else(hormones$no_start == 1, 1, hormones$`Excluded?`)
table(hormones$`Excluded?`) # n = 5032, 6 excluded
# count subjects
subject_counter <- hormones %>% rowwise() %>% mutate(DHEAmean = if_else(`Excluded?` == 1, NA, DHEAmean)) %>% group_by(src_subject_id) %>% summarise(TimepointsWithDHEAData = sum(!is.na(DHEAmean))) %>% filter(TimepointsWithDHEAData > 0) %>% left_join(sex, by = 'src_subject_id')
table(subject_counter$MasterSex) # 6083 Males, 5629 Females remain, 11712 total. -0 total

hormones$`Excluded?` <- if_else(hormones$no_end == 1, 1, hormones$`Excluded?`)
table(hormones$`Excluded?`) # n = 5070, 38 excluded
# count subjects
subject_counter <- hormones %>% rowwise() %>% mutate(DHEAmean = if_else(`Excluded?` == 1, NA, DHEAmean)) %>% group_by(src_subject_id) %>% summarise(TimepointsWithDHEAData = sum(!is.na(DHEAmean))) %>% filter(TimepointsWithDHEAData > 0) %>% left_join(sex, by = 'src_subject_id')
table(subject_counter$MasterSex) # 6082 Males, 5629 Females remain, 11711 total. -1M total

hormones$`Excluded?` <- if_else(hormones$no_freeze == 1, 1, hormones$`Excluded?`)
table(hormones$`Excluded?`) # n = 5095, 25 excluded
# count subjects
subject_counter <- hormones %>% rowwise() %>% mutate(DHEAmean = if_else(`Excluded?` == 1, NA, DHEAmean)) %>% group_by(src_subject_id) %>% summarise(TimepointsWithDHEAData = sum(!is.na(DHEAmean))) %>% filter(TimepointsWithDHEAData > 0) %>% left_join(sex, by = 'src_subject_id')
table(subject_counter$MasterSex) # 6082 Males, 5628 Females remain, 11710 total. -1F total
...
# count subjects
subject_counter <- hormones %>% rowwise() %>% mutate(DHEAmean = if_else(`Excluded?` == 1, NA, DHEAmean)) %>% group_by(src_subject_id) %>% summarise(TimepointsWithDHEAData = sum(!is.na(DHEAmean))) %>% filter(TimepointsWithDHEAData > 0) %>% left_join(sex, by = 'src_subject_id')
table(subject_counter$MasterSex) # 6078 Males, 5626 Females remain, 11704 total. -2F, -4M, -6 total
...
# count subjects
subject_counter <- hormones %>% rowwise() %>% mutate(DHEAmean = if_else(`Excluded?` == 1, NA, DHEAmean)) %>% group_by(src_subject_id) %>% summarise(TimepointsWithDHEAData = sum(!is.na(DHEAmean))) %>% filter(TimepointsWithDHEAData > 0) %>% left_join(sex, by = 'src_subject_id')
table(subject_counter$MasterSex) # 6077 Males, 5624 Females remain, 11701 total. -2F, -1M, -3 total
...
# count subjects
subject_counter <- hormones %>% rowwise() %>% mutate(DHEAmean = if_else(`Excluded?` == 1, NA, DHEAmean)) %>% group_by(src_subject_id) %>% summarise(TimepointsWithDHEAData = sum(!is.na(DHEAmean))) %>% filter(TimepointsWithDHEAData > 0) %>% left_join(sex, by = 'src_subject_id')
table(subject_counter$MasterSex) # 6071 Males, 5616 Females remain, 11687 total. -8F, -6M, -14 total
...
# count subjects
subject_counter <- hormones %>% rowwise() %>% mutate(DHEAmean = if_else(`Excluded?` == 1, NA, DHEAmean)) %>% group_by(src_subject_id) %>% summarise(TimepointsWithDHEAData = sum(!is.na(DHEAmean))) %>% filter(TimepointsWithDHEAData > 0) %>% left_join(sex, by = 'src_subject_id')
table(subject_counter$MasterSex) # 6064 Males, 5612 Females remain, 11676 total. -4F, -7M, -11 total
...
# count subjects
subject_counter <- hormones %>% rowwise() %>% mutate(DHEAmean = if_else(`Excluded?` == 1, NA, DHEAmean)) %>% group_by(src_subject_id) %>% summarise(TimepointsWithDHEAData = sum(!is.na(DHEAmean))) %>% filter(TimepointsWithDHEAData > 0) %>% left_join(sex, by = 'src_subject_id')
table(subject_counter$MasterSex) # 6063 Males, 5609 Females remain, 11672 total. -3F, -1M, -4 total
...
# count subjects
subject_counter <- hormones %>% rowwise() %>% mutate(DHEAmean = if_else(`Excluded?` == 1, NA, DHEAmean)) %>% group_by(src_subject_id) %>% summarise(TimepointsWithDHEAData = sum(!is.na(DHEAmean))) %>% filter(TimepointsWithDHEAData > 0) %>% left_join(sex, by = 'src_subject_id')
table(subject_counter$MasterSex) # 6041 Males, 5586 Females remain, 11627 total. -23F, -22M, -45 total
...
subject_counter <- hormones %>% rowwise() %>% mutate(DHEAmean = if_else(`Excluded?` == 1, NA, DHEAmean)) %>% group_by(src_subject_id) %>% summarise(TimepointsWithDHEAData = sum(!is.na(DHEAmean))) %>% filter(TimepointsWithDHEAData > 0) %>% left_join(sex, by = 'src_subject_id')
table(subject_counter$MasterSex) # Aim: 6041 Males, 5586 Females remain, 11627 total
